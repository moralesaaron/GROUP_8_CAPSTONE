...
class RegisterDorm extends Controller
{
  public function index()
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      // Handle Profile Image Upload (make it required and validated)
      if (!empty($_FILES['image']['name'])) {
        ...
        if ($_FILES['image']['error'] === 0 && in_array($_FILES['image']['type'], $allowed)) {
          ...
          if (move_uploaded_file($_FILES['image']['tmp_name'], $destination)) {
            $_POST['image'] = $filename;  // Save the uploaded image name
          } else {
            $errors[] = "Profile image upload failed. Please try again.";
          }
        } else {
          $errors[] = "Invalid image file type for profile image. Please upload a valid image (JPEG/PNG/JPG).";
        }
      } else {
        $errors[] = "Profile image is required.";  // If no image is uploaded, add an error
      }

      // No need to handle ID image at all
    ...
    $this->view('registerdorm', ['errors' => $errors]);
  }
}
